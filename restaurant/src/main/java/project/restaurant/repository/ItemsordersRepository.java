 ...
 * and ItemsordersRepository extends this collection to modify 'itemsorders' table.
 *
 * @author Tanmeet, Wen, Pengyuan(Kenny)
 */
@Repository
public interface ItemsordersRepository extends JpaRepository<ItemsOrders, Integer> {
   ...
   */
  @Query(value =
      "select id.itemid, id.quantity, price.price from (SELECT itemid,count(itemid) as quantity "
          +
          "FROM itemsorders where orderid =:orderId group by itemid order by quantity desc) "
          +
          "as id, "
          +
          "(select itemid,price from menu_items) as price where id.itemid = price.itemid",
      nativeQuery = true)
